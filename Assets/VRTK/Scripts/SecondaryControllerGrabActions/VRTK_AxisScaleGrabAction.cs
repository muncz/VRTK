...
namespace VRTK.SecondaryControllerGrabActions
{
    ...
    public class VRTK_AxisScaleGrabAction : VRTK_BaseGrabAction
    {
        ...
        private void NonUniformScale()
        {
            ...
            var newScaleX = CalculateAxisScale(grabbedObject.transform.position.x, secondaryInitialGrabPoint.position.x, secondaryGrabbingObject.transform.position.x);
            var newScaleY = CalculateAxisScale(grabbedObject.transform.position.y, secondaryInitialGrabPoint.position.y, secondaryGrabbingObject.transform.position.y);
            var newScaleZ = CalculateAxisScale(grabbedObject.transform.position.z, secondaryInitialGrabPoint.position.z, secondaryGrabbingObject.transform.position.z);

            var newScale = new Vector3(newScaleX, newScaleY, newScaleZ);

            var rotatedScale = initialScale + (grabbedObject.transform.rotation * newScale);

            var finalScaleX = (rotatedScale.x < 0 || lockXAxis ? existingScale.x : rotatedScale.x);
            var finalScaleY = (rotatedScale.y < 0 || lockYAxis ? existingScale.y : rotatedScale.y);
            var finalScaleZ = (rotatedScale.z < 0 || lockZAxis ? existingScale.z : rotatedScale.z);
        ...
        {
            return (centerPosition < initialPosition ? currentPosition - initialPosition : initialPosition - currentPosition);
        }
    }
}